...
class ApiService {
  ...
  static Future<List<dynamic>> detectPotholesFile(
    String filePath, {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('API Hatası (File): ${response.statusCode}');
        return [];
      }
    } catch (e) {
      debugPrint('Bağlantı Hatası (File): $e');
      return [];
    }
  }

  /// Görüntü çerçevesini ham Byte formatında (BGRA8888) gönderir. (0 Gecikme)
  static Future<List<dynamic>> detectPotholesRaw(
    List<int> bytes,
    int width,
    int height, {
    bool saveRecord = false,
    ...
    try {
      ...
      debugPrint('Bağlantı Hatası (Raw): $e');
      return [];
    }
  }
  ...
  static Future<bool> checkServerStatus() async {
    try {
      ...
      final data = jsonDecode(response.body);
      return data is Map<String, dynamic> && data['model_ready'] == true;
    } catch (e) {
  ...
  static void _tryStartBackend() async {
    ...
    try {
      if (Platform.isWindows) {
        ...
        // Kurulum paketindeki derlenmiş backend'i önce dene.
        final packagedBackendDir = '$exeDir\\RoadGuardBackend';
        final packagedBackendExe = '$packagedBackendDir\\RoadGuardBackend.exe';
        ...
        if (await File(venvPython).exists()) {
          ...
          debugPrint('Backend otomatik başlatıldı (Venv). Yol: $appPy');
          ...
          debugPrint(
            'Backend otomatik başlatıldı (Sistem Python). Yol: $appPy',
          );
        } else {
          debugPrint('Backend scripti (app.py) bulunamadı!');
        }
      }
    } catch (e) {
      debugPrint('Backend otomatik başlatılamadı: $e');
    } finally {
